...
def creer_permissions_pour_nouveau_role(sender, instance, created, **kwargs):
    """
    Quand un nouveau rôle est créé, on crée toutes les permissions (role, menu) à False.
    """
    if not created:
        return
    ...
    for menu in menus:
        RoleMenuAcces.objects.get_or_create(role=instance, menu=menu, defaults={"est_visible": False})


@receiver(post_save, sender=Menu)
def creer_permissions_pour_nouveau_menu(sender, instance, created, **kwargs):
    """
    Quand un nouveau menu est créé, on crée toutes les permissions (role, menu) à False.
    """
    if not created:
        return
    ...
    for role in roles:
        RoleMenuAcces.objects.get_or_create(role=role, menu=instance, defaults={"est_visible": False})
